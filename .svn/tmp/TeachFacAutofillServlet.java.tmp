...
/**
 * Trampoline servlet for selecting a record on the page <code>teachingFaculty.jsp</code>.
 * 
 * @author Gryphon Ayers (2019)
 */


@WebServlet(name="TeachFacAutofillServlet", urlPatterns = {"/TeachFacAutofillServlet"})
public class TeachFacAutofillServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        TeachingExpManager texm = new TeachingExpManager();
        ...
        if(request.getParameter("updateTeaching") != null) {
            Integer expNumber = Integer.parseInt(request.getParameter("updateTeaching"));
            if(expNumber == -1) {
                // "no degree" was selected
                sess.removeAttribute("currentexp");
            } else {
                TeachingExp e = texm.getTeachingExp(expNumber);
