...
public class DatabaseManagement implements database.DatabaseManagement {
    ...
    public void CreateTables() {
       ...
       String application = "CREATE TABLE IF NOT EXISTS `Tenure`.`application` ( " +
            ...
            "  `friendlyName` VARCHAR(90) NULL, " +
            "  `teachingNarrative` INT NULL, " +
            "  `scholarNarrative` INT NULL, " +
            "  `communityNarrative` INT NULL, " +
            "  PRIMARY KEY (`applicationID`), " +
            "  INDEX `applicantUser_idx` (`applicantUser` ASC), " +
            "  UNIQUE INDEX `applicationID_UNIQUE` (`applicationID` ASC), " +
            "  INDEX `teachingNarrative_idx` (`teachingNarrative` ASC), " +
            "  INDEX `scholarNarrative_idx` (`scholarNarrative` ASC), " +
            "  INDEX `communityNarrative_idx` (`communityNarrative` ASC), " +
            "  CONSTRAINT `applicantUser` " +
            "    FOREIGN KEY (`applicantUser`) " +
            "    REFERENCES `Tenure`.`bu_users` (`userNumber`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE NO ACTION, " +
            "  CONSTRAINT `teachingNarrative` " +
            "    FOREIGN KEY (`teachingNarrative`) " +
            "    REFERENCES `Tenure`.`narrative` (`narrativeID`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE CASCADE, " +
            "  CONSTRAINT `scholarNarrative` " +
            "    FOREIGN KEY (`scholarNarrative`) " +
            "    REFERENCES `Tenure`.`narrative` (`narrativeID`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE CASCADE, " +
            "  CONSTRAINT `communityNarrative` " +
            "    FOREIGN KEY (`communityNarrative`) " +
            "    REFERENCES `Tenure`.`narrative` (`narrativeID`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE CASCADE);";
       String profile = "CREATE TABLE IF NOT EXISTS `Tenure`.`profile` ( " +
            ...
            "  `campusPhone` VARCHAR(15) NULL, " +
            "  `yearsOfService` INT NULL, " +
            "  `rank` VARCHAR(45) NULL, " +
            "  `appointmentDate` DATETIME NULL, " +
            "  `lastPromotionDate` DATETIME NULL, " +
            "  `lastApplication` INT NULL, " +
            "  PRIMARY KEY (`profileID`), " +
            "  INDEX `profileUser_idx` (`profileUser` ASC), " +
            "  INDEX `lastApplication_idx` (`lastApplication` ASC), " +
            "  CONSTRAINT `profileUser` " +
            "    FOREIGN KEY (`profileUser`) " +
            "    REFERENCES `Tenure`.`bu_users` (`userNumber`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE NO ACTION, " +
            "  CONSTRAINT `lastApplication` " +
            "    FOREIGN KEY (`lastApplication`) " +
       ...
       String narrative = "CREATE TABLE IF NOT EXISTS `Tenure`.`narrative` ( " +
            ...
            "  CONSTRAINT `application_narrative` " +
       ...
       String degree = "CREATE TABLE IF NOT EXISTS `Tenure`.`degree` ( " +
            ...
            "  `degreeDate` VARCHAR(45) NULL, " +
            "  `transcript` INT NULL, " +
            "  PRIMARY KEY (`degreeID`), " +
            "  INDEX `application_idx` (`application` ASC), " +
            "  INDEX `transcript_idx` (`transcript` ASC), " +
            "  CONSTRAINT `application_degree` " +
            "    FOREIGN KEY (`application`) " +
            "    REFERENCES `Tenure`.`application` (`applicationID`) " +
            "    ON DELETE CASCADE " +
            "    ON UPDATE CASCADE, " +
       ...
       String localexp = "CREATE TABLE IF NOT EXISTS `Tenure`.`localexp` ( " +
            "`localexpID` INT NOT NULL AUTO_INCREMENT, " +
            "  `application` INT NOT NULL, " +
            "  `semester` ENUM('Spring', 'Summer', 'Fall', 'Winter') NULL, " +
            "  `year` INT(4) NULL, " +
            "  `courseCode` VARCHAR(10) NULL, " +
            "  `courseName` VARCHAR(60) NULL, " +
            "  `sections` VARCHAR(45) NULL, " +
       ...
       String schactivity = "CREATE TABLE IF NOT EXISTS `Tenure`.`schactivity` ( " +
            "`schActivityID` INT NOT NULL AUTO_INCREMENT, " +
            "  `application` INT NOT NULL, " +
       ...
       String svcactivity = "CREATE TABLE IF NOT EXISTS `Tenure`.`svcactivity` ( " +
            "`svcActivityID` INT NOT NULL AUTO_INCREMENT, " +
            "  `application` INT NOT NULL, " +
