...
/**
 * Trampoline servlet for populating the applications dropdown in <code>myApplication.jsp</code>.
 * @author Riley Hughes (2019)
 */
@WebServlet(name = "GetApplicationsServlet", urlPatterns = {"/GetApplicationsServlet"})
public class GetApplicationsServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // Get the user's ID from session and use that to get their applications
        BuUser u = (BuUser) request.getSession().getAttribute("user");
        Collection<Application> a = am.getApplicationsByUserID(u.getUserNumber());

        // Send applications forward!
        request.getSession().setAttribute("apps", a);
